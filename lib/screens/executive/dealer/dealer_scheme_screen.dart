...
class _DealerSchemeScreenState extends State<DealerSchemeScreen> {
  ...
  Map<String, Map<String, bool>> _expandedMonths = {}; // FY -> Month -> isExpanded
        ...
        children: [
          ...
          if (isExpanded)
            Container(
              ...
              child: Column(
                children: _buildMonthGroupsForYear(financialYear, schemes),
              ),
            ),
        ],
      ),
  ...
  List<Widget> _buildMonthGroupsForYear(String financialYear, List<Map<String, dynamic>> schemes) {
    ...
    for (var scheme in schemes) {
      ...
      if (startDate != null) {
        try {
          DateTime date = _parseDate(startDate);
          String monthYear = '${_getMonthName(date.month)} ${date.year}';

          if (!monthGroups.containsKey(monthYear)) {
            monthGroups[monthYear] = [];
          }
          monthGroups[monthYear]!.add(scheme);
        } catch (e) {
          print('Error parsing date: $startDate - ${e.toString()}');
        }
      }
    }

    // Sort months chronologically
    List<String> sortedMonths = monthGroups.keys.toList()..sort((a, b) {
      try {
    ...
    return sortedMonths.map((monthYear) {
      List<Map<String, dynamic>> monthSchemes = monthGroups[monthYear]!;
      return _buildMonthCard(financialYear, monthYear, monthSchemes);
    }).toList();
  }
  ...
  DateTime _parseDate(String dateStr) {
    // Handle DD-MM-YY format (e.g., "01-09-25")
    try {
      List<String> parts = dateStr.split('-');
      if (parts.length == 3) {
        int day = int.parse(parts[0]);
        ...
        // Assuming 00-49 is 2000-2049, 50-99 is 1950-1999
        if (year < 100) {
      ...
      print('Error parsing date parts: $dateStr - ${e.toString()}');
    }
  ...
  Widget _buildMonthCard(String financialYear, String monthYear, List<Map<String, dynamic>> schemes) {
    // Initialize expanded state for this month if not exists
    if (!_expandedMonths.containsKey(financialYear)) {
      _expandedMonths[financialYear] = {};
    }
    bool isExpanded = _expandedMonths[financialYear]![monthYear] ?? false;
    ...
    for (var scheme in schemes) {
      if (scheme['TotalAmount'] != null) {
        if (scheme['TotalAmount'] is num) {
          monthTotal += scheme['TotalAmount'].toDouble();
        } else if (scheme['TotalAmount'] is String) {
          monthTotal += double.tryParse(scheme['TotalAmount']) ?? 0.0;
        }
      }

      if (scheme['CN/VALUE'] != null) {
        if (scheme['CN/VALUE'] is num) {
          monthCN += scheme['CN/VALUE'].toDouble();
        } else if (scheme['CN/VALUE'] is String) {
          monthCN += double.tryParse(scheme['CN/VALUE']) ?? 0.0;
        }
      }
    }*/
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: ThemeHelper.accentTeal.withOpacity(0.3),
          width: 1,
        ),
      ),
      child: Column(
        children: [
          GestureDetector(
            onTap: () {
              setState(() {
                _expandedMonths[financialYear]![monthYear] = !isExpanded;
              });
            },
            child: Container(
              padding: EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: ThemeHelper.accentTeal.withOpacity(0.05),
                borderRadius: isExpanded
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: ThemeHelper.accentTeal.withOpacity(0.2),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: ThemeHelper.captionStyle(context).copyWith(fontSize: 10),
                        ),
                      ],
                    ),
                  ),
                  /*Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Icon(Icons.currency_rupee, color: ThemeHelper.infoColor, size: 12),
                          SizedBox(width: 2),
                          Text(
                      ...
                      Row(
                        children: [
                          Icon(Icons.card_giftcard, color: ThemeHelper.successColor, size: 12),
                          SizedBox(width: 2),
                          Text(
                            _formatAmount(monthCN),
                            style: ThemeHelper.captionStyle(context).copyWith(fontSize: 11),
                          ),
                        ],
                      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _buildCompactInfo("Achieved Slab", Formatters.formatNumber(data['TotalAmount']), ThemeHelper.infoColor)),
              SizedBox(width: 8),
              Expanded(child: _buildCompactInfoWithDoubleTap("Benefit",

                  (data['CN/VALUE'] is num)
                      ? Formatters.formatNumber(data['CN/VALUE'])
                      : (num.tryParse(data['CN/VALUE']?.toString() ?? '0') != null)
                      ? Formatters.formatNumber(double.tryParse(data['CN/VALUE'].toString())!)
                      : data['CN/VALUE'].toString(),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Points Earned", int.parse(data['total_earned_points'].toString().split('.')[0]).toString(), ThemeHelper.infoColor)),
                SizedBox(width: 8),
                Expanded(child: _buildCompactInfo("Current Slab", int.parse(data['current_tier_points'].toString().split('.')[0]).toString(), ThemeHelper.warningColor)),
              ],
            ),
            SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Scheme Benefit ", data['current_tier_benefit']?.toString() ?? '0', ThemeHelper.successColor)),
                SizedBox(width: 8),
                Expanded(child: _buildCompactInfo("Balance to Next Slab", pointsNeeded.toStringAsFixed(0), ThemeHelper.accentPurple)),
              ],
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Achieved",  data['row_count'].toString(), ThemeHelper.infoColor)),
                SizedBox(width: 8),
            ...
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Scheme Benefit",

                    (num.tryParse(data['current_yes_CN']?.toString() ?? '0') != null)
                        ? Formatters.formatNumber(double.tryParse(data['current_yes_CN'].toString())!)
                        : data['current_yes_CN'].toString()
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Scheme Benefit", currentCnValue, ThemeHelper.successColor)),
                SizedBox(width: 8),
                Expanded(child: _buildCompactInfo("Balance to Next Slab", _formatAmount(amountToNextSlab), ThemeHelper.accentPurple)),
              ],
            ),
          ],
        ),
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildCompactInfo("Scheme Benefit", currentCnValue, ThemeHelper.successColor)),
                SizedBox(width: 8),
                Expanded(child: _buildCompactInfo("Balance to Next Slab", qtyToNextSlab.toStringAsFixed(2), ThemeHelper.accentPurple)),
              ],
            ),
          ],
        ),
  ...
  Widget _buildCompactInfoWithDoubleTap(String title, String value, Color color) {
    return GestureDetector(
      onDoubleTap: () {
        showDialog(
          context: context,
          ...
          children: [
            Row(
              children: [
                ...
                Icon(Icons.touch_app, size: 12, color: color.withOpacity(0.5)),
              ],
            ),
            SizedBox(height: 4),
            Text(
